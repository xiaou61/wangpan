...
- 前台：资料搜索、分类筛选、夸克/百度筛选、资源需求反馈、移动端适配。
- 后台：登录、资料管理、分类管理、用户需求收集、访客记录、具体网盘地址打开数据。
...
验证会检查前台桌面/手机页面、后台登录、新增资料、前台搜索、资源需求提交、后台需求处理和测试数据清理。
...
- `data/db.json`：运行时真实数据，包含资料、分类、访客记录、打开记录、用户需求反馈，已加入 `.gitignore`。
